...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { document_id, pdf_base64, file_name } = await request.json();
    ...
    const bodyHtml = `<p>Dear ${escapeHtml(customerName)},</p><p>Please find ${escapeHtml(kind.toLowerCase())} <strong>${escapeHtml(number)}</strong> attached.</p><p>Total: <strong>${escapeHtml(document.currency)} ${Number(document.total).toFixed(2)}</strong></p><p>Kind regards,<br>IACy</p>`;
    ...
    await admin.from('audit_events').insert({ workspace_id: document.workspace_id, entity_type: document.document_type, entity_id: document.id, action: 'emailed', created_by: user.id, details: { outbox_id: outboxId, recipient } });
