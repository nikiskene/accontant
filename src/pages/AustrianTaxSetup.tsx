...
export function AustrianTaxSetup() {
  ...
  const save = async (event: FormEvent) => {
    ...
    setMessage('Saving…');
    const { data, error } = await supabase.rpc('create_austrian_tax_year', {
      p_workspace_id: workspaceId,
      p_year: Number(form.year),
      p_prior_year_revenue: Number(form.revenue),
      p_vat_regime: form.regime,
      p_accounting_method: form.method,
      p_override_frequency: form.override || null,
      p_override_reason: form.override ? 'Manual override; requires accountant confirmation' : null,
    });
    const result = Array.isArray(data) ? data[0] : data;
    setMessage(error ? error.message : `Tax year created. VAT filing workflow: ${result?.filing_frequency}.`);
  };

  if (workspace?.country !== 'AT') return <p>This setup applies only to the Austrian company.</p>;
  return <div className="max-w-3xl">
    <h1 className="text-3xl font-bold">Create Austrian tax year</h1>
    <p className="text-gray-600 mt-1 mb-6">The system derives the VAT filing cycle from prior-year revenue. Overrides remain visibly unconfirmed.</p>
    <form onSubmit={save} className="bg-white border rounded-xl p-6 space-y-4">
      ...
      <Input required type="number" min="0" step="0.01" label="Prior-year taxable revenue (EUR)" value={form.revenue} onChange={e => setForm({ ...form, revenue: e.target.value })} />
      ...
      <Button type="submit">Create and configure tax year</Button>
