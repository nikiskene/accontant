...
export function SalesDocuments() {
  ...
  const [filter, setFilter] = useState<'all'|'draft'|'quote'|'invoice'|'credit_note'>('all');
  ...
  const load = async () => {
    ...
    else if (filter !== 'all') query = query.eq('document_type', filter);
  ...
  return <div>
    ...
    <div className="flex gap-2 mb-4 overflow-x-auto">{(['all','draft','quote','invoice','credit_note'] as const).map(value=><button key={value} onClick={()=>setFilter(value)} className={`whitespace-nowrap px-3 py-2 rounded-lg text-sm capitalize ${filter===value?'bg-blue-600 text-white':'bg-white border'}`}>{value.replace('_',' ')}</button>)}</div>
    ...
    <div className="bg-white rounded-xl border overflow-x-auto"><table className="w-full min-w-[820px]"><thead className="bg-gray-50 text-left text-xs uppercase text-gray-500"><tr><th className="p-4">Number</th><th>Customer</th><th>Type</th><th>Date</th><th>Status</th><th className="text-right">Total</th><th className="pr-4 text-right">Actions</th></tr></thead><tbody>
      {documents.map(doc=><tr key={doc.id} className="border-t cursor-pointer hover:bg-gray-50" onClick={()=>{window.history.pushState({},'',`/sales-documents/${doc.id}`);window.dispatchEvent(new PopStateEvent('popstate'));}}><td className="p-4 font-medium">{doc.document_number || 'Draft'}</td><td>{doc.customer?.company_name || doc.customer?.name || 'Customer'}</td><td className="capitalize">{doc.document_type.replace('_',' ')}</td><td>{doc.issue_date}</td><td className="capitalize">{doc.status.replace('_',' ')}</td><td className="text-right">{doc.currency} {Number(doc.total).toFixed(2)}</td><td className="pr-4 text-right"><div className="flex justify-end gap-2">{doc.status==='draft'&&doc.document_type!=='credit_note'&&<Button size="sm" variant="secondary" onClick={event=>{event.stopPropagation();window.history.pushState({},'',`/new-quote?type=${doc.document_type}&draft=${doc.id}`);window.dispatchEvent(new PopStateEvent('popstate'))}}>Edit draft</Button>}{doc.document_type==='invoice'&&!['draft','void','paid'].includes(doc.status)&&Number(doc.amount_paid||0)<Number(doc.total)&&<Button size="sm" variant="secondary" onClick={event=>{event.stopPropagation();window.history.pushState({},'',`/receivables?invoice=${doc.id}`);window.dispatchEvent(new PopStateEvent('popstate'))}}>Record payment</Button>}</div></td></tr>)}
