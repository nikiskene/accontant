...
type Doc={id:string;workspace_id:string;document_type:string;document_number:string|null;issue_date:string;due_date:string|null;status:string;currency:string;subtotal:number;tax_total:number;total:number;amount_paid:number;header_text:string|null;terms_text:string|null;issuer_snapshot:any;customer_snapshot:any;customer:any};
...
export function SalesDocumentDetail({id}:{id:string}) {
  ...
  const openComposer=()=>{if(!doc)return;const customer=doc.customer||doc.customer_snapshot||{};const kind=doc.document_type==='quote'?'Cost estimate':doc.document_type==='credit_note'?'Credit note':'Invoice';const name=customer.company_name||customer.alias||'Customer';setEmail({to:customer.email||'',subject:`${kind} ${doc.document_number||''}`,body:`Dear ${name},\n\nPlease find ${kind.toLowerCase()} ${doc.document_number||''} attached.\n\nTotal: ${doc.currency} ${Number(doc.total).toFixed(2)}\n\nKind regards,\nIACy`});setComposeOpen(true)};
  const send=async()=>{const pdf=await makePdf();if(!pdf||!doc)return;setSending(true);setMsg('Sending…');const{data,error}=await supabase.functions.invoke('send-sales-document',{body:{document_id:id,pdf_base64:pdfBase64(pdf),file_name:`${doc.document_number||'document'}.pdf`,to_address:email.to,custom_subject:email.subject,body_text:email.body}});setSending(false);if(error||data?.error)setMsg(data?.error||error?.message||'Could not send email');else{setComposeOpen(false);setMsg(`Sent to ${data.recipient}.`)}};
  ...
  return <div className="max-w-5xl">
    ...
    <div className="bg-white border rounded-2xl p-4 sm:p-7">
      <div className="flex flex-col gap-4 sm:flex-row sm:justify-between"><div><p className="text-sm uppercase text-gray-500">{doc.document_type.replace('_',' ')}</p><h1 className="text-3xl font-bold">{doc.document_number||'Draft'}</h1><div className="mt-2 text-sm text-gray-600"><p className="font-medium text-gray-800">{customer.company_name||customer.alias}</p>{customer.street_address&&<p>{customer.street_address}</p>}{(customer.zip||customer.city)&&<p>{[customer.zip,customer.city].filter(Boolean).join(' ')}</p>}{customer.country&&<p>{customer.country}</p>}{customer.vat_trn&&<p>UID / VAT ID: {customer.vat_trn}</p>}</div></div><div className="sm:text-right"><span className="capitalize bg-gray-100 px-3 py-1 rounded">{doc.status.replace('_',' ')}</span><p className="text-2xl font-bold mt-3">{doc.currency} {Number(doc.total).toFixed(2)}</p>{doc.document_type==='invoice'&&Number(doc.amount_paid)>0&&<p className="text-sm text-gray-600">Paid {doc.currency} {Number(doc.amount_paid).toFixed(2)} · Open {doc.currency} {Math.max(0,Number(doc.total)-Number(doc.amount_paid)).toFixed(2)}</p>}</div></div>
      {doc.header_text&&<div className="mt-6 rounded-lg bg-gray-50 p-3 text-sm font-medium whitespace-pre-line">{doc.header_text}</div>}
      ...
      {doc.terms_text&&<div className="mt-6 text-sm"><p className="font-semibold">Payment terms</p><p className="mt-1 whitespace-pre-line text-gray-600">{doc.terms_text}</p></div>}
      <div className="flex flex-wrap justify-end gap-2 mt-7"><Button variant="secondary" onClick={download}>Download PDF</Button>{doc.status!=='draft'&&doc.status!=='void'&&<Button onClick={openComposer}>Compose email</Button>}{doc.status==='draft'&&<><Button variant="danger" onClick={remove}>Delete draft</Button><Button onClick={()=>rpc('issue_sales_document',{p_document_id:id})}>Issue document</Button></>}{doc.document_type==='quote'&&doc.status==='sent'&&<Button onClick={()=>rpc('accept_quote',{p_quote_id:id})}>Accept quote</Button>}{doc.document_type==='quote'&&['accepted','partially_invoiced'].includes(doc.status)&&<div className="flex gap-2"><Input className="w-24" type="number" min="1" max="100" value={fraction} onChange={event=>setFraction(event.target.value)}/><Button onClick={convert}>Invoice %</Button></div>}</div>
      {msg&&<p className="text-sm text-amber-700 mt-4">{msg}</p>}
    </div><Modal isOpen={composeOpen}onClose={()=>!sending&&setComposeOpen(false)}title={`Email ${doc.document_type.replace('_',' ')}`}size="lg"><div className="space-y-4"><Input type="email"required label="To"value={email.to}onChange={e=>setEmail({...email,to:e.target.value})}/><Input required label="Subject"value={email.subject}onChange={e=>setEmail({...email,subject:e.target.value})}/><div><label className="mb-1 block text-sm font-medium text-gray-700">Message</label><textarea required rows={10}value={email.body}onChange={e=>setEmail({...email,body:e.target.value})}className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/></div><p className="text-xs text-gray-500">The current PDF will be attached automatically and the sent email will be recorded in the audit trail.</p><div className="flex justify-end gap-2"><Button variant="secondary"onClick={()=>setComposeOpen(false)}disabled={sending}>Cancel</Button><Button onClick={send}disabled={sending}>{sending?'Sending…':'Send email with PDF'}</Button></div></div></Modal>
  </div>;
}
