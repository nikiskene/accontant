  ...
  return (
    <div className="max-w-3xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {receiptAddress && <form onSubmit={saveReceiptAddress} className="bg-white rounded-lg shadow p-6"><h2 className="text-xl font-semibold text-gray-900 mb-2">Your Samly receipt address</h2><p className="mb-4 text-sm text-gray-600">Choose a readable address for forwarding bills and receipts. Earlier addresses remain active, so no receipt is lost.</p><div className="flex max-w-xl items-end gap-2"><Input label="Address" value={receiptLocal} onChange={e=>setReceiptLocal(e.target.value.toLowerCase())} required /><span className="mb-2 whitespace-nowrap text-sm text-gray-600">@samly.cc</span><Button type="submit" disabled={receiptBusy}>{receiptBusy ? 'Saving...' : 'Save'}</Button></div><p className="mt-3 text-sm font-medium text-blue-800">Current: {receiptAddress}</p></form>}
