...
const blank = (billingEmail='') => ({ legal_form:'',registration_number:'',tax_identification_number:'',vat_number:'',uid_number:'',corporate_tax_number:'',trade_licence_number:'',licensing_authority:'',tax_office:'',address_line_1:'',address_line_2:'',postal_code:'',city:'',state_region:'',billing_email:billingEmail,phone:'',website:'',authorized_signatory:'',default_payment_terms_days:'14',legal_footer:'' });
const profileKeys = Object.keys(blank());

export function CompanyProfile() {
  const { user, workspaceId, workspace, refetchReferenceData } = useApp();
  const [legalName,setLegalName]=useState('');
  const [form,setForm]=useState<Record<string,string>>(()=>blank()); const [message,setMessage]=useState('');
  useEffect(()=>{setLegalName(workspace?.legal_name||'');setForm(blank(user?.email||''));if(!workspaceId)return; supabase.from('company_legal_profiles').select(profileKeys.join(',')).eq('workspace_id',workspaceId).maybeSingle().then(({data,error})=>{if(data){const values=data as unknown as Record<string,unknown>;setForm(Object.fromEntries(profileKeys.map(key=>[key,String(values[key]??'')])))}if(error)setMessage(error.message);});},[workspaceId,workspace?.legal_name,user?.email]);
  const save=async(e:FormEvent)=>{e.preventDefault();if(!workspaceId||!workspace||!legalName.trim())return;setMessage('Saving…');const profile=Object.fromEntries(profileKeys.map(key=>[key,form[key]?.trim()||null]));const{error}=await supabase.rpc('save_company_legal_profile',{p_workspace_id:workspaceId,p_legal_name:legalName.trim(),p_profile:profile});if(!error)await refetchReferenceData();setMessage(error?error.message:'Company credentials saved.');};
  ...
  return <div className="max-w-5xl"><h1 className="text-3xl font-bold">Company credentials</h1><p className="text-gray-600 mt-1 mb-6">Legal identity used for this company’s documents. Issued documents keep an immutable snapshot.</p>
    <form onSubmit={save} className="space-y-6"><section className="bg-white border rounded-xl p-6"><h2 className="text-lg font-semibold mb-4">Legal registration</h2><div className="grid md:grid-cols-2 gap-4">
      ...
      <div className="flex flex-wrap items-center gap-3"><Button type="submit">Save credentials</Button>{message&&<span className="text-sm text-gray-600">{message}</span>}</div></form></div>;
}
