...
export function CompanyProfile() {
  ...
  useEffect(()=>{ if(!workspaceId)return; supabase.from('company_legal_profiles').select(profileKeys.join(',')).eq('workspace_id',workspaceId).single().then(({data,error})=>{if(data){const values=data as unknown as Record<string,unknown>;setForm(Object.fromEntries(profileKeys.map(key=>[key,String(values[key]??'')])))}if(error)setMessage(error.message);});},[workspaceId]);
  const save=async(e:FormEvent)=>{e.preventDefault();if(!workspaceId||!workspace)return;setMessage('Saving…');const profile=Object.fromEntries(profileKeys.map(key=>[key,form[key]?.trim()||null]));const {error}=await supabase.from('company_legal_profiles').upsert({...profile,workspace_id:workspaceId,country_code:workspace.country,default_payment_terms_days:Number(form.default_payment_terms_days||14),updated_at:new Date().toISOString()});setMessage(error?error.message:'Company credentials saved.');};
